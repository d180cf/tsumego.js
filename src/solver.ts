...
interface Observer {
    solving(board: Board, color: Color): void;
    solved(board: Board, color: Color, result: Result): void;
    move(from: Board, toolbar: Board, moveBy: XYIndex): void;
}
...
function solve(
    ...
    function __solve(path: Board[], color: Color, nkotreats: number): Result {
        ...
        function _solve(color: Color): Result {
            ...
            if (observer)
                observer.solved(board, color, result);
