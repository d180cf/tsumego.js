...
module tsumego.benson {
    ...
    export function alive(b: Board, root: XY) {
        ...
        function has2eyes(r: XY, path: number[]) {
            ...
            search: for (const lib of region(r, (t, s) => sameColor(s) && b.inBounds(t.x, t.y))) {
                // the region(...) above enumerates stones in the chain and the liberties
                if (b.chainAt(lib.x, lib.y))
                    continue;

                const libertyId = ++nRegions;
                ...
                for (let i = 0; i < adjacent.length; i++) {
                    ...
                    if (path.indexOf(ch) < 0 && !has2eyes(adjacentXY[i], [...path, ch]))
                        continue search;
                }
